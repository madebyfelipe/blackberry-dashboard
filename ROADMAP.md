...
1. **Felipe confirma a persistência em produção** e recria o Blob store como Private (issue #39). O Neon e o Blob já estão no ar e conectados em Production, então o modo memória acabou; falta ver uma arte sobreviver ao próximo deploy (fecha a issue #11) e trocar o store público por um privado — a Vercel não converte, tem que criar outro, migrar as artes e trocar `BLOB_READ_WRITE_TOKEN`.
