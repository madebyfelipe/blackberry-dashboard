...
Documento vivo. Marca o que já existe, o que falta **desenhar** (você) e o que falta **construir** (Claude), até o fim do projeto. Última atualização: **22 set 2026** (Inbox, issue #30 — a conversa do time, sem a camada de tempo real ainda).
...
- ✅ **Inbox — a conversa do time** (issue #30, export "Inbox"): grupos e diretas estilo Discord numa tela só, histórico salvo e pesquisável (na lista e dentro da conversa), não lidas, silenciar, presença (disponível · ocupado · ausente · offline, símbolo por **forma**, trocada no menu da sua conta) e a chamada no popup dos dois avatares, que deixa o registro no histórico. Só o time da agência entra; cliente segue decidindo pelo link público. `src/lib/inbox/*` + `src/components/inbox/*` + `/api/inbox/*`
- 🟡 **Tempo real do Inbox**: não existe WebSocket/WebRTC no projeto. Mensagem nova chega por releitura periódica (12s, só com a aba à vista) e a chamada conta o tempo sem transmitir — microfone e tela aparecem desligados, dizendo por quê. É a próxima decisão técnica desta área (o Felipe já disse que a notificação de chamada recebida — toast, som, fora do app — ele providencia)
- ⬜ Notificações, Conversas, Relatórios, Membros (hoje placeholders — todos aparecem na lateral do v3)
- ✅ **Testes automatizados** (`tests/`, runner do próprio Node, 323 casos das funções puras, incluindo o isolamento entre agências e a régua de clientes) **+ CI** (`.github/workflows/ci.yml`: tipos, testes e build a cada push e pull request)
...
5. **Camada de tempo real** (a decisão técnica que o Inbox destravou): WebSocket para mensagem e presença no lugar da releitura de 12s, e WebRTC para a chamada transmitir de verdade (áudio + tela). O resto da tela do Inbox não muda quando ela entrar.
